...
if atCNRM:
    cproject('erai', ('grid', '_'),
             ('frequency', 'monthly'))  # no grid writes as '_' , otherwise as e.g. 'T42' or 'T127'

    root = "/cnrm/amacs/DATA/OBS/netcdf/${frequency}"
    # for original grid
    patmonth1 = root + "_mean/erai/erai_???_mm_${variable}${grid}${PERIOD}.nc"
    # for other grids e.g. : grid ='T42' or 'T127'
    patmonth2 = root + "_mean/erai/erai_???_mm_${variable}.${grid}.${PERIOD}.nc"
    ...
    # calias("erai",'sfcWind','fg10',filenameVar='10FG')
    # #sfcWind:="Near-Surface Wind Speed" et fg10:="10 metre wind gust"
