...
#
# Mean on latitude axis: you must set only one point (xpoint, ypoint or zpoint) because rank=4 and a mean on latitude is
# done
#

# Plot a Hovmoller diagram on all domain, at level index 3, and using %c for 'fmt'
# (i.e. small month abbreviation e.g., Jun):
...
#
# Mean on longitude axis: you must set only one point (xpoint, ypoint or zpoint) because rank=4 and a mean on longitude
# is done
#
...
diag8 = hovm(tas, title='2m_Temperature', mean_axis='Lat', fmt="%C",
             **hovm_params(
                 'NINO4'))  # => diag (x,t). If you use xpoint/ypoint/zpoint, selected points are not considered.
...
diag9 = hovm(tas, title='2m_Temperature', mean_axis='Lon', fmt="%C",
             **hovm_params(
                 'NINO4'))  # => diag (y,t). If you use xpoint/ypoint/zpoint, selected points are not considered.
...
diag11 = hovm(tas, title='2m_Temperature', mean_axis='Point', ypoint=2, fmt="%C",
              **hovm_params('NINO4'))  # => diag (x,t)
...
diag12 = hovm(ta_zonmean, title='Zonal mean', mean_axis='Lat', fmt="%C",
              **hovm_params(
                  'GRL'))  # => diag (z,t). If you use xpoint/ypoint/zpoint, selected points are not considered.

#
# No mean: section at different points => you have to select only one point (ypoint or zpoint) because rank=3
#
diag13 = hovm(ta_zonmean, title='Zonal mean', mean_axis='Point', ypoint=2, fmt="%C",
              **hovm_params('GRL'))  # => diag (z,t)
diag14 = hovm(ta_zonmean, title='Zonal mean', mean_axis='Point', zpoint=1000., fmt="%C",
              **hovm_params('GRL'))  # => diag (y,t)
...
diag15 = hovm(ta_mermean, title='Meridional mean', mean_axis='Lon', fmt="%C",
              **hovm_params(
                  'NATL'))  # => diag (z,t). If you use xpoint/ypoint/zpoint, selected points are not considered.

#
# No mean: section at different points => you have to select only one point (xpoint or zpoint) because rank=3
#
diag16 = hovm(ta_mermean, title='Meridional mean', mean_axis='Point', xpoint=360., fmt="%C",
              **hovm_params('NATL'))  # => diag (z,t)
diag17 = hovm(ta_mermean, title='Meridional mean', mean_axis='Point', zpoint=2000., fmt="%C",
              **hovm_params('NATL'))  # => diag (x,t)
