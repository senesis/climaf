...
for pathfilename in filenames_list:
    ...
    # cdftime = netcdftime.utime(t_unit, calendar=t_cal)
    # datevar.append(num2date(time_test, units=t_unit, calendar=t_cal))
    # datevar.append(num2date(time_test, units=cdftime, calendar=t_cal))
    # datevar.append(cdftime.num2date(time_test))
    # datevar.append(cdftime.dates.num2date(time_test))
    #
    # x = np.array(datevar)[0,:]
    # datevaro = datevar[0][:]
    # x = list(datevaro)
    ...
    if args.highlight_period:
        ...
        plt.plot(x[ind], y[ind], lw=highlight_period_lw_list[dataset_number], color=colors[dataset_number])
    #
...
# -- Add the titles
if args.left_string:
    plt.title(args.left_string, loc='left', fontsize=(float(args.left_string_fontsize) if args.left_string_fontsize
                                                      else default_left_string_fontsize))

if args.center_string:
    plt.title(args.center_string, loc='center', fontsize=(float(args.center_string_fontsize)
                                                          if args.center_string_fontsize
...
# -- X and Y axis labels
if args.xlabel:
    plt.xlabel(args.xlabel, fontsize=(float(args.xlabel_fontsize)
                                      if args.xlabel_fontsize else default_xlabel_fontsize))
if args.ylabel:
    plt.ylabel(args.ylabel, fontsize=(float(args.ylabel_fontsize)
                                      if args.ylabel_fontsize else default_ylabel_fontsize))
...
if draw_legend:
    ...
    leg_dict = dict(bbox_to_anchor=(float(str.split(legend_xy_pos, ',')[0]), float(str.split(legend_xy_pos, ',')[1])),
                    loc=int(legend_loc), borderaxespad=0., prop={'size': float(legend_fontsize)},
                    ncol=int(legend_ncol), frameon=legend_frame)
...
if args.text:
    ...
    text_verticalalignment_list = (str.split(args.text_verticalalignment, ',')
                                   if args.text_verticalalignment else ['bottom'] * len(text_list))
    ...
    text_horizontalalignment_list = (str.split(args.text_horizontalalignment, ',')
                                     if args.text_horizontalalignment else ['left'] * len(text_list))
